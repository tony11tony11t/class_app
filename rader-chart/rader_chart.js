function paintrader_chart() {
    ...
    var bg_pos = new Array(6);

    $("rader-chart").each(function (e) { //取得HTML中的所有rader-chart標籤
        $(this).html("");
        Big_size = parseInt($(this).css("width"));
        size = Big_size;
        ...
        for (var j = 0; j < 6; j++)
            bg_pos[j] = new Array(2);
        bg_pos[0][0] = LeftAndRight_margin;
        bg_pos[0][1] = Top_margin;
        bg_pos[1][0] = LeftAndRight_margin + start_width;
        bg_pos[1][1] = Top_margin;
        bg_pos[2][0] = LeftAndRight_margin + start_width * 1.5;
        bg_pos[2][1] = Top_margin + polygon_height;
        bg_pos[3][0] = LeftAndRight_margin + start_width;
        bg_pos[3][1] = Top_margin + polygon_height * 2;
        bg_pos[4][0] = LeftAndRight_margin;
        bg_pos[4][1] = Top_margin + polygon_height * 2;
        bg_pos[5][0] = LeftAndRight_margin - start_width * 0.5;
        bg_pos[5][1] = Top_margin + polygon_height;
        ...
        //畫上背景五條連到中間的線
        for (var i = 0; i < 3; i++) {
            var line_to_center = "M" + bg_pos[i][0] + " " + bg_pos[i][1] +
                "L" + bg_pos[i + 3][0] + " " + bg_pos[i + 3][1];
            path[i + 5] = document.createElementNS('http://www.w3.org/2000/svg', "path"); //建立svg標籤
            path[i + 5].setAttribute("d", line_to_center);
            path[i + 5].setAttribute("class", "inside_line");
            svg.appendChild(path[i + 5]);
        }
        //畫上分數
